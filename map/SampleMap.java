...
public class SampleMap implements Map, Serializable {
  ...
  public SampleMap(ImageSize size, OpticalProperties opticalProperties,
    ...
    radialC = opticalProperties.getCoef(2);
  ...
  public void map(double[] in, double[] out) {
    ...
    double rr = x * x + y * y;
    double radial = radialA + rr * (radialB + rr * radialC);
  ...
  public void unmap(double[] in, double[] out) {
    ...
    double c0 = rrr * rrr * radialC;
    double a1 = 1 * radialA;
    double b1 = 3 * rrr * radialB;
    double c1 = 5 * rrr * rrr * radialC;
    ...
    for (;;) {
      ...
      double next =
          radialInvFactor - (-1 + radialInvFactor * (a0 + sq * (b0 + sq * c0)))
              / (a1 + sq * (b1 + sq * c1));

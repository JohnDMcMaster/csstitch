...
public class QuadTreeChecker2 {
  ...
  public void checkMapping() throws IOException {
    if (!(position >= mappingPosition && position < mappingPosition + MAPPING_SIZE)) {
      ...
      mapping = channel.map(FileChannel.MapMode.READ_WRITE, mappingPosition, Math.min(MAPPING_SIZE
          + MAPPING_BUFFER, channel.size() - mappingPosition));
    }
  }
  ...
  public QuadTreeChecker2(String filename) throws IOException {
